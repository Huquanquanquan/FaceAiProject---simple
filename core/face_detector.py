...
# from core.detection import detector // 放到下面局部导入了,导入太多了造成循环导入
...
# 添加图片到test_image中，实现随意添加
def select_image(image_dir=None):
    root = tk.Tk()
    root.withdraw()  # 隐藏主窗口
    file_path = filedialog.askopenfilename(title='选择人脸检测图像', filetypes=[("Image files", "*.jpg;*.jpeg;*.png")])
...
#
# Select -> face_detection -> clear_images 实现选中检测完就删除
#
